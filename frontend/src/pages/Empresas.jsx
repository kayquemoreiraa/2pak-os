...
const ORIGENS = [{value:"indicacao",label:"Indicacao"},{value:"inbound",label:"Inbound"},{value:"outbound",label:"Outbound"},{value:"evento",label:"Evento"},{value:"redes_sociais",label:"Redes Sociais"}];
const formInicial = { nome_fantasia:"", razao_social:"", cnpj:"", segmento:"", porte:"", site:"", origem_lead:"", status_atual_id:"" };
...
export default function Empresas() {
  ...
  function abrirEditar(emp) { setEditandoId(emp.id); setForm({nome_fantasia:emp.nome_fantasia||"",razao_social:emp.razao_social||"",cnpj:emp.cnpj||"",segmento:emp.segmento||"",porte:emp.porte||"",site:emp.site||"",origem_lead:emp.origem_lead||"",status_atual_id:emp.status_atual_id||""}); setErro(""); setModal(true); }
    ...
    <div className="animate-fade-in">
      ...
      : filtradas.length === 0 ? (
        <div className="text-center py-20">
          ...
          <button onClick={abrirCriar} className="mt-4 text-cortex-primary text-sm hover:underline">Adicionar primeira empresa</button>
        </div>
      ) : (
            ...
            <div key={emp.id} className="cortex-card-hover p-5 flex items-center justify-between group">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-cortex-subtle text-xs">{emp.segmento||"Sem segmento"}</p>
                </div>
              </div>
      ...
      {modal && (
        <Modal titulo={editandoId?"Editar Empresa":"Nova Empresa"} onClose={fecharModal}>
          <div className="flex flex-col gap-4">
            ...
            <Input label="Razao Social" name="razao_social" placeholder="Ex: Cortex Web Ltda" value={form.razao_social} onChange={handleChange}/>
            <div className="grid grid-cols-2 gap-3">
              <Input label="CNPJ" name="cnpj" placeholder="00.000.000/0001-00" value={form.cnpj} onChange={handleChange}/>
              <Input label="Segmento" name="segmento" placeholder="Ex: Tecnologia" value={form.segmento} onChange={handleChange}/>
            </div>
            <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={handleSalvar} disabled={salvando} className="cortex-btn-primary flex-1 disabled:opacity-50">
